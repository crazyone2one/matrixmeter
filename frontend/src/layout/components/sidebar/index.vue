...
import {renderIconStr, renderRouterLink} from "/@/utils/render.ts";
  ...
  if (_route) {
    for (const route of _route) {
      ...
      const menu: MenuOption = {
        key: String(name),
        label: () => children?.length ? t(meta?.locale as string) : renderRouterLink(String(name), meta?.locale as string),
        icon: await renderIconStr(
            meta?.icon as keyof typeof import("@vicons/ionicons5")
        ),
      };
      if (children?.length) {
            ...
            ...(menu.children || []),
            {
              ...
              label: () =>
                  el.meta?.hideChildrenInMenu ?
                      renderRouterLink(getFirstRouterNameByCurrentRoute(name as string) as string, meta?.locale as string)
...
onBeforeMount(async () => {
  options.value = await travel(menuTree.value);
});
</script>
<template>
  <n-layout-sider
