package cn.master.matrix.handler.aspect;
...
public class OperationLogAspect {
     ...
     */
    @Pointcut("@annotation(cn.master.matrix.handler.annotation.Log)")
    ...
    public void before(JoinPoint joinPoint) {
        try {
            ...
            if (msLog != null && isMatch(msLog.type())) {
                ...
                for (Class<?> clazz : msLog.mmClass()) {
                    context.setVariable("mmClass", applicationContext.getBean(clazz));
                    isNext = true;
                }
                if (!isNext) {
                    return;
    ...
    public String getId(Object result) {
        try {
            if (result != null) {
                ...
                Map object = JsonUtils.parseMap(resultStr);
    ...
    public void saveLog(JoinPoint joinPoint, Object result) {
        try {
            ...
            log.error("操作日志写入异常：{}", e.getMessage());
        } finally {
            localUser.remove();
            beforeValues.remove();
        }
